...
interface Course {
  id: number;
  title: string;
  category: 'Data' | 'Cloud' | 'Development' | 'Testing' | 'Networking' | 'Management';
  duration: string;
  level: string;
  description: string;
  image: string;
...
export default function Courses() {
  ...
  const tabs = ['All', 'Data', 'Cloud', 'Development', 'Testing', 'Networking', 'Management'];
  ...
  const courses: Course[] = [
    ...
    {
      id: 4,
      title: "Full Stack  with AI Development",
            ...
            <div className="bg-white p-2 rounded-3xl shadow-sm border border-slate-100 flex lg:flex-col flex-row gap-1">
              {tabs.map((tab) => (
                <button
                  ...
                  className={`flex-1 px-6 py-4 rounded-2xl text-sm font-bold transition-all flex items-center justify-between group ${activeTab === tab
